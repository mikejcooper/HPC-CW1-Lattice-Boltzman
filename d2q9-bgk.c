    ...
    for (int jj = 0; jj < params.nx; jj++)
    {
      ...
      ** respecting periodic boundary conditions (wrap around) */
      ...
      // tmp_cells[y_s * params.nx + x_e].speeds[8]  = cells[index].speeds[8]; /* south-east */
            ...
            tmp_cells[ii * params.nx + jj].speeds[0]    = cells[index].speeds[0]; /* central cell, no movement */
      tmp_cells[ii * params.nx + (jj + 1) % params.nx].speeds[1]   = cells[index].speeds[1]; /* east */
      tmp_cells[(ii + 1) % params.ny * params.nx + jj].speeds[2]   = cells[index].speeds[2]; /* north */
      tmp_cells[ii * params.nx + ((jj == 0) ? (jj + params.nx - 1) : (jj - 1))].speeds[3]   = cells[index].speeds[3]; /* west */
      tmp_cells[(ii == 0) ? (ii + params.ny - 1) : (ii - 1) * params.nx + jj].speeds[4]   = cells[index].speeds[4]; /* south */
      tmp_cells[(ii + 1) % params.ny * params.nx + (jj + 1) % params.nx].speeds[5]  = cells[index].speeds[5]; /* north-east */
      tmp_cells[(ii + 1) % params.ny * params.nx + ((jj == 0) ? (jj + params.nx - 1) : (jj - 1))].speeds[6]  = cells[index].speeds[6]; /* north-west */
      tmp_cells[(ii == 0) ? (ii + params.ny - 1) : (ii - 1) * params.nx + ((jj == 0) ? (jj + params.nx - 1) : (jj - 1))].speeds[7]  = cells[index].speeds[7]; /* south-west */
      tmp_cells[(ii == 0) ? (ii + params.ny - 1) : (ii - 1) * params.nx + (jj + 1) % params.nx].speeds[8]  = cells[index].speeds[8]; /* south-east */
    }
  }
}
