...
// --------END-------------------------------------------------------
      ...
      else
      {
       ...
       /* compute x velocity component */
        ...
        cells[index].speeds[0] = temp.speeds[0]
                                                  + params.omega
                                                  * (w0 * local_density * (1.0 - u_sq / (c_sq + c_sq))
                                                    - temp.speeds[0]);
        cells[index].speeds[1] = temp.speeds[1]
                                                  + params.omega
                                                  * (w1 * local_density * (1.0 + u_x / c_sq + (u_x * u_x) / e1 - e2)
                                                    - temp.speeds[1]);
        cells[index].speeds[2] = temp.speeds[2]
                                                  + params.omega
                                                  * (w1 * local_density * (1.0 + u_y / c_sq + (u_y * u_y) / e1 - e2)
                                                    - temp.speeds[2]);
        cells[index].speeds[3] = temp.speeds[3]
                                                  + params.omega
                                                  * (w1 * local_density * (1.0 + - u_x / c_sq + (- u_x * - u_x) / e1 - e2)
                                                    - temp.speeds[3]);
        cells[index].speeds[4] = temp.speeds[4]
                                                  + params.omega
                                                  * (w1 * local_density * (1.0 + - u_y / c_sq + (- u_y * - u_y) / e1 - e2)
                                                    - temp.speeds[4]);
        cells[index].speeds[5] = temp.speeds[5]
                                                  + params.omega
                                                  * (w2 * local_density * (1.0 + (u_x + u_y) / c_sq + ((u_x + u_y) * (u_x + u_y)) / e1 - e2)
                                                    - temp.speeds[5]);
        cells[index].speeds[6] = temp.speeds[6]
                                                  + params.omega
                                                  * (w2 * local_density * (1.0 + (- u_x + u_y) / c_sq + ((- u_x + u_y) * (- u_x + u_y)) / e1 - e2)
                                                    - temp.speeds[6]);
        cells[index].speeds[7] = temp.speeds[7]
                                                  + params.omega
                                                  * (w2 * local_density * (1.0 + (- u_x - u_y) / c_sq + ((- u_x - u_y) * (- u_x - u_y)) / e1 - e2)
                                                    - temp.speeds[7]);
        cells[index].speeds[8] = temp.speeds[8]
                                                  + params.omega
                                                  * (w2 * local_density * (1.0 + (u_x - u_y) / c_sq + ((u_x - u_y) * (u_x - u_y)) / e1 - e2)
                                                    - temp.speeds[8]);
